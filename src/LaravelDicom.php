...
class LaravelDicom
{
    ...
    */
    public function parse($infile)
    {
        ...
        $fh = @fopen($infile, "rb");
        if (!$fh) {
            throw new Exception("Could not open file $infile for reading");
        }
        $stat = fstat($fh);
        ...
        {
            $element = new Element($fh, $this->dict);
    ...
    public function write($outfile = '')
    {
        if ($outfile == '') {
            if (isset($this->current_file)) {
                ...
                throw new Exception("File name not given (and no file currently open)");
            }
        }
        $fh = @fopen($outfile, "wb");
        if (!$fh) {
            throw new Exception("Could not open file $outfile for writing");
        }

        // Writing file from scratch will always fail for now
        if (!isset($this->_preamble_buff)) {
            throw new Exception("Cannot write DICOM file from scratch");
        }
        ...
        foreach (array_keys($this->_elements) as $group) {
            foreach (array_keys($this->_elements[$group]) as $element) {
                ...
                if ($this->_elements[$group][$element]->vr_type == self::VR_TYPE_EXPLICIT_32_BITS ||
                     $this->_elements[$group][$element]->vr_type == self::VR_TYPE_EXPLICIT_16_BITS) {
                ...
                } elseif ($this->_elements[$group][$element]->vr_type == self::VR_TYPE_IMPLICIT) {
                    fwrite($fh, pack('V', $this->_elements[$group][$element]->length));
                }
                switch ($code) {
                    ...
                    case 'OX':
                        ...
                        if (!$fh2) {
                            throw new Exception("Could not open file {$this->current_file} for reading");
                        }
    ...
    */
    public function getValue($gp_or_name, $el = null)
    {
        if (isset($el)) // retreive by group and element index
        {
            if (isset($this->_elements[$gp_or_name][$el])) {
                ...
                throw new Exception("Element ($gp_or_name,$el) not found");
            }
        }
        else // retreive by name
        {
            if (isset($this->_elements_by_name[$gp_or_name])) {
                ...
                throw new Exception("Element $gp_or_name not found");
            }
        }
    }
    ...
    */
    public function dumpImage($filename)
    {
        ...
        fwrite($fh, "P5\n");
        // comment
        fwrite($fh, "# file generated by Mkinyua53/Laravel-Dicom on ". date("Y-m-d H:i:s") ." \n");
